...
```{r}
raw_data = read.csv("./fig.1bcd.csv")
...
```{r}
library(linkET)
model1 =  read_excel("./model.xlsx",sheet = 1) %>% select( -1 ) 
phy = read_excel("./model.xlsx",sheet = 2) %>% select( 2:10 )
